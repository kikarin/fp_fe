...
function FAQItem({ q, a, isOpen, onToggle }: { q: string; a: string; isOpen: boolean; onToggle: () => void }) {
  return (
    <motion.div
      className="border border-[#EEEEEE] rounded-[16px] overflow-hidden bg-white"
    ...
    >
      <button
        ...
        onClick={onToggle}
      >
        <span className="text-[15px] font-semibold text-[#111111]">{q}</span>
        <motion.span
          className="flex-shrink-0 w-7 h-7 rounded-full border border-[#E0E0E0] flex items-center justify-center text-[#111]"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="px-6"
          >
            <p className="text-[14px] text-[#666666] leading-[1.7] pb-5">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-24 bg-[#F9F9F9] border-y border-[#EEEEEE] px-8">
      <div className="max-w-[1030px] mx-auto">
        ...
        >
          <span className="text-[12px] font-semibold tracking-[0.15em] text-[#2F6BFF] uppercase">FAQ</span>
